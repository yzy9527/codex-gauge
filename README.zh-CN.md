...
Codex Gauge 无需打开 Codex，即可展示 5 小时与一周额度、各自的重置时间和可用的使用限制重置次数。菜单栏优先反映 5 小时剩余百分比，并在该窗口不可用时自动回退到其他可用窗口；弹出面板则按系统本地时区显示详细状态。
...
- 直接在菜单栏展示 Codex 的 5 小时剩余额度，并在该窗口不可用时自动回退。
- Codex 同时提供两个窗口时，展示 5 小时和一周额度百分比及各自的重置时间。
- 展示可用重置次数及到期明细。
